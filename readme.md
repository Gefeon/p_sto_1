...
- Теперь проект может быть сконфигурирован на основе нескольких профилей: _dev_ и _local_
- По умолчанию подключен профиль _local_. Для переключения профиля необходимо в файле application.properties изменить:
````
spring.profiles.active=local
````
- Профиль _local_ - создан для запуска проекта с локальными настройками БД. Для этого необходимо добавить системные переменные:
